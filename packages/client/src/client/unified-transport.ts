/**
 * @lens/client - Unified Transport (WebSocket)
 *
 * WebSocket implementation of UnifiedTransport for unified client.
 * Implements the operations protocol with field-level subscriptions.
 */

import type { UnifiedTransport } from "./unified";
...
/** WebSocket unified transport options */
export interface WebSocketUnifiedTransportOptions {
	/** WebSocket URL */
	url: string;
...
/** Transport state */
export type WebSocketUnifiedState = "connecting" | "connected" | "disconnected" | "reconnecting";
 ...
 * Implements the operations protocol with field-level subscriptions.
 */
export class WebSocketUnifiedTransport implements UnifiedTransport {
	private ws: WebSocket | null = null;
	private state: WebSocketUnifiedState = "disconnected";
	...
	/** Options */
	private options: Required<WebSocketUnifiedTransportOptions>;

	/** Reconnection state */
	private reconnectAttempts = 0;
	private reconnectTimer: ReturnType<typeof setTimeout> | null = null;

	constructor(options: WebSocketUnifiedTransportOptions) {
		this.options = {
			reconnectDelay: 1000,
	...
	getState(): WebSocketUnifiedState {
		return this.state;
	}
}
...
/**
 * Create WebSocket unified transport
 */
export function createWebSocketUnifiedTransport(
	options: WebSocketUnifiedTransportOptions,
): WebSocketUnifiedTransport {
	return new WebSocketUnifiedTransport(options);
}

/**
 * Create WebSocket unified transport (alias)
 */
export function websocketUnifiedTransport(
	options: WebSocketUnifiedTransportOptions,
): WebSocketUnifiedTransport {
	return new WebSocketUnifiedTransport(options);
}
