...
interface PusherClient {
	trigger(
		...
		event: string,
		data: unknown,
	): Promise<unknown>; // Response from node-fetch
}
...
export function pusher(options: PusherTransportOptions): SubscriptionTransport {
	const { appId, key, secret, cluster, useTLS = true, channelPrefix = "lens-", debug = false } = options;
	...
	return {
		...
		async init(): Promise<void> {
			...
			try {
				...
				const Pusher = (PusherModule as unknown as { default: new (opts: unknown) => PusherClient }).default ?? PusherModule;
				client = new Pusher({
					appId,
					key,
					secret,
